...
// import { resolveTypeReferenceDirective } from '../../../node_modules/typescript';
class OrderTrack extends React.Component {
  ...
  componentDidMount() {
    ...
    if (!window.localStorage.getItem("openid")) {
      window.location.href =
        "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx89ac1808e298928d&redirect_uri=http%3a%2f%2fwww.freelycar.com%2ffreelycar_wechat%2fapi%2fuser%2fwechatlogin%3FhtmlPage%3Dordertrack%2f%24%26isAuth%3dtrue&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect";
    }
    //通过后台对微信签名的验证。
    getWXConfig({
  ...
  render() {
    ...
    let projectItems = this.state.projects.map((item, index) => {
      let price =
        item.projectInfo.presentPrice +
        item.projectInfo.pricePerUnit * item.projectInfo.referWorkTime;
      let projectPrice =
      ...
      for (let inventoryItem of this.state.inventoryInfos) {
        if (inventoryItem.projectId === item.projectInfo.projectId) {
          projectPrice =
            projectPrice + inventoryItem.number * inventoryItem.inventory.price;
        ...
        >
          <Flex style={{ width: "100%" }}>
            ...
            <div style={{ marginLeft: "auto" }}>
              <span style={{ fontSize: ".24rem" }}>￥</span>
              {price}
            </div>
          </Flex>
          ...
          {item.projectInfo.payMethod == "0" && (
            <Flex className="order-track-remain" style={{ width: "100%" }}>
              <div>
                <p>
                  已抵扣会员卡
                  {item.projectInfo.cardNumber}
                  ，该项目还剩余
                  {item.remaining}次
                </p>
              </div>
              <p style={{ marginLeft: "auto", fontSize: ".22rem" }}>
                <span style={{ fontSize: ".22rem" }}>￥</span>
                {projectPrice}
                <i />
              </p>
            </Flex>
          )}
          {item.projectInfo.payMethod == "2" && (
            <Flex className="order-track-remain" style={{ width: "100%" }}>
              <div>
                <p>
                  已抵扣
                  {item.projectInfo.favourName}
                  ，该项目还剩余
                  {item.remaining}次
                </p>
              </div>
    ...
    return (
      <div className="body-bac">
        ...
        {!this.state.empty && (
          <div>
            <Flex className="order-track-baseinfo">
              <Flex.Item className="Info">
                <p>
                  ...
                  {window.localStorage.getItem("isMember") === "true"
                    ? "(会员)"
                    : ""}
                </p>
            ...
            <Flex className="order-track-program" direction="column">
              ...
              >
                <div style={{ marginRight: ".42rem", display: "inline-block" }}>
                  {this.state.payState == 1 ? "实付" : "合计"}：
                  <span style={{ fontSize: ".24rem", color: "#e42f2f" }}>
                    ￥
                  </span>
                  <span style={{ color: "#e42f2f" }}>
                    {this.state.payState == 1
            ...
            <div
              className="order-list order-tarck-info"
              style={{ marginTop: "0", borderTop: "1px dashed #f0f0f0" }}
            >
              <Flex style={{ height: "100%" }}>
            ...
            <div className="order-track-state">
              <Flex
                className={
                  this.state.state >= 2
                    ? "order-track-state-box active"
                    : "order-track-state-box"
                }
                align="start"
              >
                <div className="time">
                  <p>
                    {this.state.deliverTime
                      ? this.state.deliverTime.slice(5, 10)
                      : ""}
                  </p>
                  <p style={{ marginLeft: ".22rem" }}>
                    {this.state.deliverTime
                      ? this.state.deliverTime.slice(11, 16)
                      : ""}
                  </p>
                </div>
                <div className="right-box">
                  ...
                  {this.state.state >= 2 &&
                    this.state.stars == 0 && (
                      <div>爱车已交回你的手中 快来评价获积分吧</div>
                    )}
                  {this.state.state >=2  &&
                    this.state.stars > 0 && (
                      <div>小易爱车竭诚为您服务 期待您的再次光临！</div>
                    )}
                  {this.state.state >= 2 &&
              ...
              {/* <div >
                <Flex
                  style={{display: (this.state.fromCabinet?"display":"none")}}
                  className={
                    this.state.state >= 1
                      ? "order-track-state-box-scan active"
                      : "order-track-state-box-scan"
                  }
                  align="start"
                >
                  <div className="time">
                    <p>
                      {this.state.finishTime
                        ? this.state.finishTime.slice(5, 10)
                        : ""}
                    </p>
                    <p style={{ marginLeft: ".22rem" }}>
                      {this.state.finishTime
                        ? this.state.finishTime.slice(11, 16)
                        : ""}
                    </p>
                  </div>
                  <div className="right-box">
                    <div className="circle" />
                    <div className="car-state">扫一扫打开柜门, 取走钥匙 ></div>
                  </div>
                </Flex>
              </div> */}
              ...
              >
                ...
                <div className="right-box">
                  <div className="circle" />
                  <div className="car-state">已完工</div>
                  {this.state.state >= 1 && (
                    <div>您的汽车已爱护完毕，期待您的带回</div>
                  )}
                </div>
              </Flex>
              <Flex
              ...
              >
                <div className="time">
                  <p>
                    {this.state.createDate
                      ? this.state.createDate.slice(5, 10)
                      : ""}
                  </p>
                  <p style={{ marginLeft: ".22rem" }}>
                    {this.state.createDate
                      ? this.state.createDate.slice(11, 16)
                      : ""}
                  </p>
                </div>
                <div className="right-box">
                  <div className="circle" />
                  <div className="car-state">商家已接车</div>
                  <div>汽车已入库</div>
                </div>
              </Flex>
              <Flex
              ...
              >
                ...
                <div className="right-box last">
                  <div className="circle" />
                  <div className="car-state">已下单</div>
                  <div />
                </div>
              </Flex>
            </div>
        ...
        {this.state.empty && <div className="empty-bac" />}
      </div>
    );
  }
}
